...
public class Layout extends JFrame implements ActionListener {
	...
	int height_first;
	int height_second;
	double TinhtongDienTich = 0;
	private Graphics g;
	private JLabel hienThiDienTich;
	...
	public void init() {
		...
		JMenuItem tongDienTich = new JMenuItem("Tong dien tich", KeyEvent.VK_T);
		hienThiDienTich = new JLabel();
		tongDienTich.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_T, InputEvent.CTRL_DOWN_MASK));
		tongDienTich.addActionListener(new ActionListener() {

			@Override
			public void actionPerformed(ActionEvent e) {
				// TODO Auto-generated method stub
				hienThiDienTich.setText("Dien tich cac hinh la " + TinhtongDienTich);
				revalidate();
			}
			
		});
		file.add(tongDienTich);
		file.add(exit);
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (count == 1) {
					...
					if (choseShape.equalsIgnoreCase("Line")) {
						...
						TinhtongDienTich += Math.sqrt((width_second - width_first) * (width_second - width_first)
								+ (height_second - height_first) * (height_second - height_first));
					} else if (choseShape.equalsIgnoreCase("Circle")) {
						g.drawOval(width_first, height_first, width_second - width_first, width_second - width_first);
						TinhtongDienTich += Math.PI*( width_second - width_first)* width_second - width_first;
						...
						TinhtongDienTich += (width_second - width_first)*(height_second - height_first);
					}
				}
		...
		this.add(hienThiDienTich, BorderLayout.NORTH);
